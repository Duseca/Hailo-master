...
import '../views/supportmodel.dart';
...
class supportChatController extends GetxController {
  ...
  final lastChats = <SupportChat>[].obs;
  ...
  Stream<List<SupportChat>> fetchLastChats() {
    Stream<QuerySnapshot> stream = chatSupportCollection
        .doc(uid!)
        .collection("chats")
        ...
        .snapshots();
    var res = stream.map(
            (qShot) => qShot.docs.map((doc) => SupportChat.fromDocument(doc)).toList());
    return res;
  }
  ...
  sendMessage() async {
    ...
    await chatSupportCollection.doc(uid).update({
      "chatStarted":true,
    "lastMessageAt" :DateTime.now()
    });
    await chatSupportCollection.doc(uid!).collection("chats").add({
  ...
  senderView(SupportChat chat) => Column(
  ...
  receiverView(SupportChat chat) => Column(
